...
本プライバシーポリシーは、TABETA（以下、「当開発者」）が開発したGoogleChromeの拡張機能(Extension)(以下、「拡張機能」とします。)の利用において、利用者の個人情報もしくはそれに準ずる情報を取り扱う際に、当開発者が遵守する方針を示したものです。
...
**ただし、個人情報には該当しない情報を利用者のブラウザのLocalStorageに保存します。**

#### 取得方法

拡張機能が対象とするページ内に、ボタン(疑似リンク)を表示します。
ボタンを押すと、関連する要素のIDがブラウザ内のLocalStorageに保存されます。
...
- 既読対象とする投稿のID
- 非表示対象とするグループまたはユーザーのID

これにより、利用者にとって不要な情報を非表示にすることが可能になり、利便性が向上します。
**これらの情報は当開発者に送信されず拡張機能利用者のブラウザのLocalStorageに保存されます**
